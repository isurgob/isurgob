...
<div id="busquedaAvanzada<?= $id; ?>">
	...
	<table border="0">
		<tr>
			...
			<td class="opcionBusquedaAvanzada<?= $id; ?> <?= $model->opcion != $valorOpcion('obj_id') ? 'hidden' : null ?>" data-opcion="<?= $valorOpcion('obj_id') ?>">
				<?= Html::activeTextInput($model, 'obj_id', ['id' => 'txObj_id' . $id, 'maxlength' => 8, 'style' => 'width:100%;', 'class' => 'form-control criterio']); ?>
			</td>
...
function clickFilaBusquedaAvanzada<?= $id; ?>( codigoObjeto, nombreObjeto, $fila ){
	...
	/**
	 * Si la fila seleccionada tiene la clase "success" indica que se hizo doble click sobre la misma fila.
	 * Se debe evitar cargar más de una vez la misma información.
	 */
