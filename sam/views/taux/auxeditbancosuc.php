...
<div class="site-auxedit">
   ...
   // --------------------------------------- Fin del comboBox --------------------------------------------
    ...
    Pjax::begin(['id' => 'idGrid']);// comienza bloque de grilla
	 ...
	 Modal::begin([
		'id' => 'ModalEdicion',
		'header' => '<h2 id="ttModal">Consulta Sucursal</h2>',
		'closeButton' => [
		  'label' => '<b>X</b>',
	...
	<div class="form" style='padding:15px 5px; margin-bottom:5px'>
		...
		<tr height='35px'>
			...
			<td>
				<?= Html::input('text', 'bco_ent', $bco_ent, ['class' => 'form-control','id'=>'bco_ent' ,'onchange' => '$.pjax.reload({container:"#bancoEntidadBancaria",data:{bco_ent:this.value,consulta:$("#txAccion").val()},method:"POST"})', 'maxlength'=> '4','style'=>'width:40px;', 'readonly' => true]); ?>
			</td>
		...
		<div  id='labelModif' style='float:right;margin-top:6px;'> <label> Modificación: </label> </div>
...
function CargarControles(bco_ent,bco_suc,nombre,domi,tel,modif)
{
	...
	function btnModificarAuxBancoSuc(bco_ent,bco_suc,nombre,domi,tel,modif){
		...
		$("#ttModal").html("Modificar Sucursal");
		$("#ModalEdicion").modal("show");
	...
	function btnEliminarAuxBancoSuc(bco_ent,bco_suc,nombre,domi,tel,modif){
			...
			$("#ttModal").html("Eliminar Sucursal");
			$("#ModalEdicion").modal("show");
