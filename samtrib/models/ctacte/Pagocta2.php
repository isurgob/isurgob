...
class Pagocta extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['trib_id', 'subcta', 'anio', 'cuota'], 'integer'],
            [['monto'], 'number'],
     ...
     */
    public function validarPeriodoVenc($trib_id,$anio,$cuota)
    {
     ...
     */
    public function cargarCuenta($trib_id,$obj_id,$anio,$cuota,$montoParcial,$fchvenc,$fchcaja)
    {
    	$session = new Session;
    	
    	$ctacte_id = Yii::$app->db->createCommand("SELECT ctacte_id FROM ctacte WHERE trib_id=" . $trib_id . " and obj_id='" . $obj_id . "' and anio=" . $anio . " and cuota=" . $cuota)->queryScalar();
    	
    	$sql = "select 0,cta_id,cta_nom,monto From sam.Uf_CtaCte_PagoParcial(" . $ctacte_id . ",";
        $sql .= $montoParcial . ",'" . Fecha::usuarioToBD($fchvenc) . "','" . Fecha::usuarioToBD($fchcaja) . "')";
        
        $data = Yii::$app->db->createCommand($sql)->queryAll();
        
        $monto = 0; //La variable monto almacenará el total de los montos
        ...
        foreach ($data as $array)
        {
			...
			$ar = [];	//Creo un arreglo vacío
			
			$ar[$array['cta_id']] = $array;	//Asigno al arreglo vacío un array de tres elementos
     ...
     * @param integer $cuota Cuota
     */
    public function existeEmiDJ($trib_id,$obj_id,$anio,$cuota)
    {
     ...
     */
     public function grabarPagocta()
     {
     	...
     	if ($ttrib != 3)
     	{
     		if ($this->trib_id != 1 && $this->trib_id != 3 && $this->validarPeriodoVenc($this->trib_id,$this->anio,$this->cuota) == 0)
     			return "No existen vencimientos del Tributo para el período ingresado.";
     		
     		if ($this->trib_id != 1 && $this->trib_id != 3 && $this->existePagoAnual($this->trib_id,$this->obj_id,$this->cuota) == 1)
     			return "No se puede realizar el pago. Existe Liquidación con Pago Anual.";
     		
     	}
     	
     	if ($this->existePerJudConv($this->trib_id,$this->obj_id,$this->anio,$this->cuota) == 1)
     		return "El período está en Juicio o Convenio.";
		...
		$session->open();
     	...
     	try
     	{
     		$pago_id = Yii::$app->db->createCommand("select nextval('seq_ctacte_pagocta')")->queryScalar();
			...
			{
				$sql = "insert into ctacte_pagocta_det values(" . $pago_id . "," . $array["cta_id"] . "," . $array["monto"] . ")";				
			...
			if (!is_numeric($this->monto)) 
     		...
     		$sql = "INSERT INTO ctacte_pagocta values(" . $pago_id . "," . $this->trib_id . ",'" . $this->obj_id . "',";
		...
		$session->close();
 		...
 		$arreglo = ['pago' => $pago_id, 'resultado' => ''];
 		return $arreglo;
