   ...
   public $domicilio;
	...
	public function grabar($expe='',$obs='')
	{
		...
		if ($cantidad == 0)
		{
			...
			$sql .= ",".intVal($this->loc_id). ",'".$this->cp."',".$this->barr_id;
			$sql .= ",".$this->calle_id.",'".substr(utb::ComillasSimples($this->nomcalle),0,40)."',".$this->puerta;
			...
			$sql .= ",calle_id=".$this->calle_id.",nomcalle='".substr(utb::ComillasSimples($this->nomcalle),0,40);
	...
	public function cbioDomicilio($taccion, $expe, $obs, $distrib, $tdistrib)
	{
        ...
        //return count($this->getDirtyAttributes());
		//se comprueba que el domicilio sea distinto
		if(
            count($this->getDirtyAttributes()) == 0
            /*
    ...
    public static function cargarDomi($origen,$obj_id,$id)
    {
    	...
    	if ($model !== null)
    	{
    		...
    		$model->pais_id = utb::getCampo("domi_provincia","prov_id=".(integer)$model->prov_id,"pais_id");
    		$model->domicilio = Domi::getDomicilio($origen,$obj_id,$id);
			
			if ($model->nomcalle == '')
				$model->nomcalle = utb::getCampo("v_domi","torigen='" . $origen . "' and id=" . $id . " and obj_id='".$model->obj_id."'","nomcalle");
    	}
        else $model= new Domi();
    ...
    public function armarDescripcion(){
    	...
    	$sql= "Select pais_id From domi_provincia Where prov_id In (Select prov_id From domi_localidad Where loc_id = " . intVal($this->loc_id) . ")";
		...
		if($localidadActual != $this->loc_id){
			$this->domicilio .= ' -' . utb::getCampo("domi_localidad","loc_id=".intVal($this->loc_id),"nombre");
			$this->domicilio .=	' - '.utb::getCampo("domi_provincia","prov_id=".(integer)$this->prov_id,"nombre");

			if($paisActual != $paisElegido)
				$this->domicilio .= ' - ' . utb::getCampo('domi_pais', "pais_id = " . intVal($paisElegido) , 'nombre');
		}
