...
class LiquidaController extends Controller
{
    ...
    public function actionView($id=0)
    {
		...
		if ($arrayCtaCte==null or !in_array(utb::getTTrib($arrayCtaCte['trib_id']), [3,4,6])) {
    ...
    public function actionCreate()
    {
		...
		$arrayCtaCte['cuota'] = 0;
		$arrayCtaCte['fchvenc'] = Fecha::bdToUsuario(date('d/m/Y'));
		...
		if (isset($_POST['txCtaCte_Id'])) 
        {
            ...
            $arrayCtaCte['fchvenc'] = (isset($_POST['fchvenc']) ? Fecha::bdToUsuario($_POST['fchvenc']) : '');
		...
		if (isset($_POST['pjTribSelec']))
		{
			...
			if ($ttrib == 3 and $arrayCtaCte['tobj'] == 1) $arrayCtaCte['fchvenc'] = Fecha::bdToUsuario(date('d/m/Y'));
		...
		if (isset($_POST['pjGetPeriodo']))
		{
			...
			$arrayCtaCte['fchvenc'] = Fecha::bdToUsuario($_POST['venc']);
			...
			{ 
				$arrayCtaCte['fchvenc'] = Fecha::bdToUsuario($model->GetFechaVenc($trib,$arrayCtaCte['anio'],$arrayCtaCte['cuota']));	
			}
		}
		...
		if ($arrayCtaCte['cuota'] == 0 or $arrayCtaCte['cuota'] == '') $arrayCtaCte['cuota'] = 1;
		return $this->render('view',[
				'arrayCtaCte' => $arrayCtaCte,
				'dataProviderItem' => $dataProviderItem,
				'consulta' => 0,
    ...
    public function actionUpdate($ctacte, $menu=0)
    {
		...
		if (isset($_POST['txCtaCte_Id'])) 
        {
            ...
            $arrayCtaCte['anio'] = (isset($_POST['txAnio']) ? $_POST['txAnio'] : 0);
            $arrayCtaCte['cuota'] = (isset($_POST['txCuota']) ? $_POST['txCuota'] : 0);
            $arrayCtaCte['fchvenc'] = Fecha::bdToUsuario((isset($_POST['fchvenc']) ? $_POST['fchvenc'] : ''));
            $arrayCtaCte['expe'] = (isset($_POST['txExpe']) ? $_POST['txExpe'] : '');
            $arrayCtaCte['obs'] = (isset($_POST['txObs']) ? $_POST['txObs'] : '');
            $ucm = $model->getUCM($arrayCtaCte['trib_id']);//(isset($_POST['opUCM']) ? $_POST['opUCM'] : 1);
		...
		if (isset($_POST['pjBtnItem']))
		{
			...
			$ItemDef = $model->CargarItemModif($item_id);
