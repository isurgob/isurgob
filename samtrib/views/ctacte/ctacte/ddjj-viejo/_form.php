...
<table border="0">
	<tr>
		...
		<td colspan="5">
			...
			<?=
				Html::activeDropDownList( $model, 'trib_id', utb::getAux('trib','trib_id','nombre',0,"tipo = 2 AND dj_tribprinc = trib_id AND est = 'A'"), [
					'id'=>'ddjj_dlTrib',
		...
		<td colspan="2">
			<?=
				Html::activeInput('text', $model, 'obj_id',[
					'id'=>'ddjj_txObjetoID',
					'class'=>'form-control ' . ( $permiteModificarObj ? '' : ' solo-lectura' ),
		...
		<td>
			...
			Modal::begin([
				...
				'toggleButton' => [
					...
					'class' 	=> 'bt-buscar',
					'style'		=> ( $permiteModificarObj ? 'display:block' : 'display:none' ),
				],
				'closeButton' => [
				  'label' => '<b>X</b>',
...
<div id="ddjj_divGrillas">
	...
	<?php Pjax::begin([ 'id' => 'PjaxCargarRete', 'enableReplaceState' => false, 'enablePushState' => false ]); ?>
	...
	<div class="div_grilla" style="margin-top: 8px;">
		...
		if( $model->hasErrors() && Yii::$app->request->get( '_pjax', '' ) == '#PjaxCargarRete' ){
...
<!-- FIN Div Grillas -->
	...
	<?php
		...
		if( $habilitarPresentar ){

			echo '<script>f_habilitarPresentarDJ();</script>';
	...
	<div id="ddjj_divBotonesShow" class="pull-left">
		...
		<?php
			...
			if( !$permiteModificarObj ){
				...
				echo Html::a('Cancelar',['listadj', 'id' => $model->obj_id ],['class' => 'btn btn-primary']);
...
<!-- INICIO Ventana Modal "Nueva Retención" -->
 <?php
 	...
 	Pjax::begin(['id' => 'ddjj_PjaxModalRete', 'enablePushState' => false, 'enableReplaceState' => false ]);
		...
		echo DdjjController::nuevaRete( $obj_id, "#ddjj_PjaxAgregarRete", "#modalNuevaRetencion" );
