...
if (!$externo)
{
	$this->params['breadcrumbs'][] = ['label' => 'Declaración Jurada'];
} else 
{
	$this->params['breadcrumbs'][] = ['label' => 'DDJJ','url' => ['view']];
	$this->params['breadcrumbs'][] = ['label' => 'Declaración Jurada'];
	$this->params['breadcrumbs'][] = ['label' => 'Comercio: '.$obj_id,'url' => ['//objeto/comer/view','id'=>$obj_id]];
}
...
<div class="ddjj-compara">
	
	<?php if ($externo){  ?>
		<table width='100%'>
			<tr>
				<td><h1>Declaración Jurada Comercio: <?= $obj_id ?></td>
				<td align='right'>
					<?= Html::a('Nuevo', ['view', 'consulta' => 0, 'action'=>0, 'obj_id'=>$obj_id],['class'=>'btn btn-primary']); ?> 
					<?= Html::a('Comercio', ['//objeto/comer/view', 'id' => $obj_id],['class'=>'btn btn-success']); ?>
				</td>
			</tr>
		</table>
	<?php } else {?>
...
<?php
	...
	$obj_id = Yii::$app->request->get('obj_id',$obj_id);
	$est = Yii::$app->request->get('est','');
	
	$dataProviderDDJJ = $model->buscarListObj($trib_id,$obj_id,$est,false,$fiscaliza);
