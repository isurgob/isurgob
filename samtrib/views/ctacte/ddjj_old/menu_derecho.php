...
<ul id='ulMenuDer' class='menu_derecho'>
	...
	<li id='liNuevo' class='glyphicon glyphicon-plus'> <?= Html::a('<b>Nuevo</b>', ['view','consulta'=>0,'action'=>0,'objeto' => $model->obj_id], ['class' => 'bt-buscar-label', 'data' => ['method' => 'post']])  ?></li>
	<?php } ?>
	...
	<?php } ?>
	<?php if (utb::getExisteProceso(3331) || utb::getExisteProceso(3332)) {?>
	<li><hr style="color: #ddd; margin:1px" /></li>
	<li id='liCargarRete' class='glyphicon glyphicon-list'> <?= Html::a('<b>Cargar Rete.</b>', ['rete','dj_id' => $model->dj_id], ['class' => 'bt-buscar-label']) ?></li>
	<li><hr style="color: #ddd; margin:1px" /></li>
	<?php } ?>
	<?php if (utb::getExisteProceso(3330)) {?>
...
} else
{
	...
	{	// habilito todas las opciones
		...
		{	//Deshabilito eliminar
			...
			//Deshabilito Cagar Retenciones
			echo '<script>$("#liCargarRete").css("pointer-events", "none");</script>';
			echo '<script>$("#liCargarRete").css("color", "#ccc");</script>';
			echo '<script>$("#liCargarRete a").css("color", "#ccc");</script>';
		}
