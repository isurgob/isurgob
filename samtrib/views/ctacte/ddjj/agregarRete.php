...
?>
 ...
 function f_grabarDetalle(){
     ...
     if( fecha == '' ){
         ...
         f_ing = new Date( array_fecha[2], array_fecha[1]-1, array_fecha[0] );
		 var cuotaref = cuota;
		 var f_max;

		 // Armo la fecha limite segun el periodo
		 if (cuotaref==1 || cuotaref==3 || cuotaref==5 || cuotaref==7 || cuotaref==8 || cuotaref==10 || cuotaref==12) {
			 f_max = new Date( anio, cuotaref-1, 31);
		 };
		 if (cuotaref==4 || cuotaref==6 || cuotaref==9 || cuotaref==11) {
			 f_max = new Date( anio, cuotaref-1, 30);
		 };
		 if (cuotaref==2) {
			 f_max = new Date( anio, 1, 28);
		 };

         if ( ( f_ing ) > ( f_max ) ){

             error.push( "La fecha ingresada es mayor a la fecha permitida. ( Fecha permitida: " + f_max.toLocaleDateString('en-GB') + ")");
 ...
 function actualizarMontoDetalle(){
 	...
 	var alicuota = isNaN( parseFloat($("#alicuotaDetalle").val() ) ) ? 0 : parseFloat($("#alicuotaDetalle").val());
 	var monto = isNaN( parseFloat($("#montoDetalle").val() ) ) ? 0 : parseFloat($("#montoDetalle").val());

 	if(!isNaN(base) && !isNaN(alicuota) && base > 0 && alicuota > 0 )
 		monto = base * alicuota / 100;

	if(!isNaN(monto) && !isNaN(alicuota) && monto > 0 && alicuota > 0 )
 		base = monto / (alicuota / 100);

 	$("#montoDetalle").val( monto.toFixed( 2 ) );
    $("#baseDetalle").val( base.toFixed( 2 ) );
    $("#alicuotaDetalle").val( alicuota.toFixed( 2 ) );
 }
