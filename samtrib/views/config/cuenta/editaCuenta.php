...
echo Html::input('hidden','Cuenta[part_id]',$model->part_id);
?>
...
<table>
	<tr>
		...
		<td colspan="3">
			<?= Html::input('text','Cuenta[part_id]',$model->part_id,[
		...
		<td>
			<?php
				...
				Pjax::begin(['id' => 'PjaxCambiaPartida'.$consulta]);
					...
					$part_id = Yii::$app->request->post( 'cuenta_part_id', $model->part_id );
					...
					{
						$res = Yii::$app->db->createCommand("SELECT EXISTS (SELECT 1 FROM fin.part WHERE formato = '" . $part_id . "' OR formatoaux = '" . $part_id . "' and anio=extract(year from current_date))")->queryScalar();

						if ( $res )	//Si existe la cuenta, obtener el número de cuenta
							$part_id = utb::getCampo( 'fin.part', "(formato = '" . $part_id . "' OR formatoaux = '" . $part_id . "') and anio=extract(year from current_date)", 'nropart' );
						else
							$part_id = 0;

					} else	//Verificar según número de partida
					{
						$res = Yii::$app->db->createCommand("SELECT EXISTS (SELECT 1 FROM fin.part WHERE part_id = " . $part_id . ")")->queryScalar();
					}

					echo Html::input('text','Cuenta[part_nom]',utb::getCampo('fin.part','nropart = ' . $part_id . ' and anio=extract(year from current_date)','nombre'),[
					...
					$res = ( intVal($part_id) == 0 ? false : $model->validarEstadoPadre( $part_id ) );
	...
	<tr>
		...
		<td><?= Html::input('text','Cuenta[cta_id]',$model->cta_id,[
		...
		<td></td>
		<td><?= Html::input('text','Cuenta[nombre_redu]',$model->nombre_redu,[
...
<table>
	<tr>
		...
		<td><?= Html::input('text','Cuenta[nombre]',$model->nombre,[
...
<table>
	<tr>
		...
		<td><?= Html::dropDownList('Cuenta[tcta]',$model->tcta,utb::getAux('ctacte_tcta','cod','nombre',0),[
...
<table>
	...
	<tr>
		<td width="115px" style="padding-left:10px">
			<?=
				Html::radio('Cuenta[cta_id_atras]',$model->cta_id == $model->cta_id_atras,[
...
<table>
	<tr>
		<td width="115px" style="padding-left:10px">
			<?= Html::radio('Cuenta[cta_id_atras]',$model->cta_id != $ctaPartida,[
		...
		<td><?= Html::dropDownList('dlOtraCuenta',($ctaPartida == $model->cta_id ? 0 : $ctaPartida),utb::getAux('fin.part','part_id','nombre',1),[
