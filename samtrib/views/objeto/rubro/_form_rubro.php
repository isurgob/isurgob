	...
	<div>
		<table border="0">
			<tr>
				...
				<td colspan="6">
					<?=
						Html::activeDropDownList( $model, 'nomen_id', $arrayNomeclador, [
							'id'		=> 'rubro_form_dlNomeclador',
							'class'		=> 'form-control' . ( $action != 0 ? ' solo-lectura' : '' ),
							'style'		=> 'width: 99%',
							'onchange'	=> 'f_rubro_cambiaNomeclador( $( this ).val(), true )',
...
function f_rubro_cambiaNomeclador( nomen_id, borrarDatos ){

	ocultarErrores( "#rubro_form_errorSummary" );

	if( borrarDatos || nomen_id == 0 ){
		$( "#rubro_form_txRubroID" ).val( '' );
		$( "#rubro_form_txRubroNom" ).val( '' );
	}

	$( "#rubro_form_txRubroID" ).toggleClass( 'solo-lectura', nomen_id == 0 || <?= $action ?> != 0 );
	$( "#rubro_form_txRubroNom" ).toggleClass( 'solo-lectura', nomen_id == 0 || <?= $action ?> != 0 );

	//se actualiza la URL del autocomplete
	$("#rubro_form_txRubroNom").autocomplete({
		source: "<?= BaseUrl::toRoute('//objeto/rubro/sugerenciarubro'); ?>&nomen_id=" + nomen_id,
	});
...
function f_rubro_cambiaIDRubro( rubro_id ){
	...
	var nomen_id = $("#rubro_form_dlNomeclador option:selected").val();

	$.get("<?= BaseUrl::toRoute('//objeto/rubro/nombrerubro'); ?>&rubro_id=" + rubro_id + "&nomen_id=" + nomen_id )
	.success( function( data ){
		datos = jQuery.parseJSON(data);
		
		if( datos.rubro_nom == '' ){

			mostrarErrores( ["El Nomeclador ingresado no es válido."], "#rubro_form_errorSummary" );
			$("#rubro_form_txRubroNom").val( "" );
			$("#rubro_form_txRubroID").val( "" );
			...
			$("#rubro_form_txRubroNom").val( datos.rubro_nom );
			$("#rubro_form_txRubroID").val( datos.rubro_id );
...
$( document ).ready( function() {
	...
	f_rubro_cambiaNomeclador( nomen_id, false );
